 ...
 */
template <typename T>
__device__ T log_cdfLap(T x, T alpha) {
    T val = x / alpha;
    if (val <= 0) {
        return log(static_cast<T>(0.5)) + val;
    }
    return log(static_cast<T>(2) - exp(-val)) + log(static_cast<T>(0.5));
}
...
/*
 * CUDA kernel for logarithm of probability computation.
 * This kernel computes the logarithm of the probability of the forward past for each batch and column.
 *
 * @param input: Input tensor of shape (batch_size, n).
 * @param output: Input tensor output of shape (batch_size, m).
 * @param log_prob: Output tensor log_prob of shape (batch_size, m, n).
 * @param alpha: Scale parameter of the Laplace distribution.
 */
template <typename T>
__global__ void log_probability_kernel(
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> input,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> output,
    at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> log_prob, const T alpha) {
    const int bs = blockIdx.y;
    ...
    if (bs < input.size(0) && index < n * m) {
        ...
        log_prob[bs][j][l] = log_cdfLap(output[bs][j] - input[bs][l], alpha);
    }
}
...
/**
 * CUDA kernel to compute components of the Jacobian-vector product (JVP) for the logarithm of Soft-Top-k algorithm.
 *
 * @param input: Input tensor of shape (batch_size, n).
...
__global__ void derivative_components_jvp_kernel(
    ...
    at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> scalar_prod_input, const T alpha) {
    const int bs = blockIdx.y;
    const int n = input.size(1);
    const int m = output.size(1);
 ...
 */
template <typename T>
__device__ T auxiliary_function(T x) {
    return (x > 0.5) ? 1 / x - 1 : static_cast<T>(1);
}

/**
 * CUDA kernel to calculate components of vector-Jacobian product (VJP) for the logarithm of Soft-Top-k algorithm.
 *
 * @param input: Input tensor of shape (batch_size, n).
 * @param output: Output tensor of Lap-Sum function with shape (batch_size, m).
 ...
 * @param batch_sum: Temporary tensor of shape (batch_size, m).
 * @param scalar_prod: Temporary tensor of shape (batch_size, m).
 * @param scalar_prod_input: Temporary tensor of shape (batch_size, m).
 * @param grad: Gradient tensor of shape (batch_size, m, n).
 * @param alpha: Scale parameter for the Laplace distribution.
 */
template <typename T>
__global__ void derivative_components_vjp_kernel(
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> input,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> output,
    const at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> log_prob,
    const at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> v,
    at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> batch_sum,
    at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> scalar_prod,
    ...
    if (bs < input.size(0) && index < n * m) {
        ...
        T value = pdfLap(output[bs][j] - input[bs][i], alpha);
        atomicAdd(&batch_sum[bs][j], value);
        value *= input[bs][i];
        atomicAdd(&scalar_prod_input[bs][j], value);

        value = exp(log_prob[bs][j][i]);
...
/**
 * CUDA kernel to calculate Jacobian-vector product (JVP) for the logarithm of Soft Top-K algorithm.
 *
...
__global__ void derivative_jvp_kernel(
    ...
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> input,
    const at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> log_prob,
    const at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> v, const T alpha) {
    const int bs = blockIdx.y;
    const int m = log_prob.size(1);
    const int n = log_prob.size(2);

    int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (bs < log_prob.size(0) && index < n * m) {
        int l = index % n;
        int j = index / n;

        T val_temp = exp(log_prob[bs][j][l]);
        val_temp = auxiliary_function(val_temp);

        T batch_sum = sum[bs][j];
        batch_sum = (batch_sum != 0 ? batch_sum : 1); // Avoid division by zero

        T value = val_temp * v[bs][j][l] / (batch_sum * alpha);
        atomicAdd(&grad_w[bs][j], value);

        value = (scalar_prod[bs][j] / batch_sum - v[bs][j][l]) * val_temp / alpha;
        grad[bs][j][l] = value;

        value = (input[bs][l] - scalar_prod_input[bs][j] / batch_sum) * val_temp * v[bs][j][l] / (alpha * alpha);
        atomicAdd(&grad_alpha[0], value);
    }
}

/**
 * CUDA kernel to calculate the vector-Jacobian product (VJP) function for the logarithm of Soft-Top-k algorithm.
 *
 * @param grad: Gradient tensor of shape (batch_size, m, n) that will be return.
 * @param grad_w: Gradient tensor of shape (batch_size, m) that will be return.
 * @param grad_alpha: Gradient tensor of shape (1) that will be return.
 ...
 */
template <typename T>
__global__ void derivative_vjp_kernel(
    at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> grad,
    at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> grad_w,
    at::PackedTensorAccessor32<T, 1, at::RestrictPtrTraits> grad_alpha,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> input,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> output,
    const at::PackedTensorAccessor32<T, 3, at::RestrictPtrTraits> v,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> batch_sum,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> scalar_prod,
    const at::PackedTensorAccessor32<T, 2, at::RestrictPtrTraits> scalar_prod_input, const T alpha) {
    const int bs = blockIdx.y;
    const int n = input.size(1);
    const int m = output.size(1);
    ...
    if (bs < input.size(0) && index < n * m) {
        ...
        T _sum = batch_sum[bs][j];
        _sum = (_sum != 0 ? _sum : 1); // Avoid division by zero

        T value = grad[bs][j][i];
        T value1 = value * v[bs][j][i] / (_sum * alpha);
        atomicAdd(&grad_w[bs][j], value1);

        value1 = pdfLap(output[bs][j] - input[bs][i], alpha);
        grad[bs][j][i] = (scalar_prod[bs][j] / _sum * value1 - v[bs][j][i] * value) / alpha;

        value1 = (input[bs][i] - scalar_prod_input[bs][j] / _sum) * value * v[bs][j][i] / (alpha * alpha);
        atomicAdd(&grad_alpha[0], value1);
    }
}

/**
 * Jacobian-vector product (JVP) function for the logarithm of Soft-Top-k algorithm and
 * the gradient of the logarithm of probability with respect to all parameters.
 *
 * @param input: Input tensor of shape (batch_size, n).
 * @param output: Output tensor of Lap-Sum function with shape (batch_size, m).
 * @param log_prob: Logarithm of probability tensor of shape (batch_size, m, n).
 ...
 */
std::vector<at::Tensor> jvp_cuda(
    at::Tensor input, at::Tensor output, at::Tensor log_prob, at::Tensor v, const float alpha) {
    const auto batch_size = input.size(0);
    const auto n = input.size(1);
    ...
    auto grad_w = at::zeros_like(output);
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grad_cuda", ([&] {
        derivative_jvp_kernel<scalar_t><<<num_blocks, threads>>>(
            ...
            input.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
            log_prob.packed_accessor32<scalar_t, 3, at::RestrictPtrTraits>(),
            v.packed_accessor32<scalar_t, 3, at::RestrictPtrTraits>(), static_cast<scalar_t>(alpha));
    }));
...
/**
 * Vector-Jacobian product (VJP) function for the logarithm of Soft-Top-k algorithm and
 * the gradient of the logarithm of probability with respect to all parameters.
 *
 * @param input: Input tensor of shape (batch_size, n).
 * @param output: Output tensor of Lap-Sum function with shape (batch_size, m).
...
std::vector<at::Tensor> vjp_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "components_cuda", ([&] {
        derivative_components_vjp_kernel<scalar_t><<<num_blocks, threads>>>(
            input.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
            output.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
            log_prob.packed_accessor32<scalar_t, 3, at::RestrictPtrTraits>(),
            v.packed_accessor32<scalar_t, 3, at::RestrictPtrTraits>(),
            ...
            scalar_prod.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
            scalar_prod_input.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
            grad.packed_accessor32<scalar_t, 3, at::RestrictPtrTraits>(), static_cast<scalar_t>(alpha));
    }));

    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grad_cuda", ([&] {
        derivative_vjp_kernel<scalar_t><<<num_blocks, threads>>>(
            grad.packed_accessor32<scalar_t, 3, at::RestrictPtrTraits>(),
            grad_w.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
            grad_alpha.packed_accessor32<scalar_t, 1, at::RestrictPtrTraits>(),
            input.packed_accessor32<scalar_t, 2, at::RestrictPtrTraits>(),
