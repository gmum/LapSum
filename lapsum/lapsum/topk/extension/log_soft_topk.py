...
from ..extension import log_soft_topk_cpu as ext_cpu
from ..extension import log_soft_topk_cuda as ext_cuda


class LogSoftTopK(Function):
    @staticmethod
    def forward(ctx, x, k, alpha, dim, largest):
        """
        Performs the forward pass for the Log-Soft-Top-k function.
        ...
        Returns:
            prob: The logarithm of probabilities tensor with the same shape as x.
        """
        x_dims = list(x.shape)
        ...
        k_dims.pop(dim)
        assert k_dims == x_dims, f"k dimensions mismatch: k{k.shape}, x{original_shape}"
        ...
        # Compute output
        if x.is_cuda:
            output, log_prob = ext_cuda.forward(r, w, alpha)
        else:
            output, log_prob = ext_cpu.forward(r, w, alpha)

        # Save for backward
        ctx.save_for_backward(r, output, log_prob)
        ctx.alpha = alpha
        ctx.dim = dim
        ...
        if ctx.original_shape is not None:
            ...
            log_prob = log_prob.view([*[original_shape[i] for i in perm_dims[:-1]], -1])
            inv_perm = [perm_dims.index(i) for i in range(x.dim())]
            log_prob = log_prob.permute(*inv_perm)
    ...
    def backward(ctx, grad_output):
        """
        Performs the backward pass for the Log-Soft-Top-k function.
        ...
        r, output, log_prob = ctx.saved_tensors
        alpha = ctx.alpha
        dim = ctx.dim
        ...
        if r.is_cuda:
            grad_r, grad_w_flat, grad_alpha = ext_cuda.vjp(
                r, output, log_prob, grad_output_flat.contiguous(), alpha
            )
        else:
...
def log_soft_topk(
    ...
    """
    Computes the Log-Soft-Top-k operation with automatic dimension handling and input validation.
    ...
    Args:
        ...
        largest: If True, selects largest elements; if False, selects smallest

    Returns:
        torch.Tensor: Log-Soft top-k logarithm of probabilities with the same shape as input x

    Examples:
        >>> x = torch.randn(2, 5)  # batch_size=2, features=5
        >>> k = torch.randn(2, 3)  # different size along dim 1
        >>> log_probs = log_soft_topk(x, k, alpha=0.1, dim=1)
    """
    # Automatically handle dimension wrapping for negative dim values
    dim = dim if dim >= 0 else x.dim() + dim

    return LogSoftTopK.apply(x, k, alpha, dim, largest)
